...
import type { PhysicalTimePoint, SessionRecord } from '../types/mindpulseExport'
    ...
    <section className="mp-card mp-analysis-stack">
      ...
      <section className="mp-subsection mp-subsection--timeline">
        ...
        <MultiLayerTimeline
          ...
          rawPhysical={session.physicalTimeSeries}
...
function MultiLayerTimeline({
  ...
  rawPhysical: PhysicalTimePoint[] | undefined
  sessionKey: string
}) {
  ...
  const rowH = 34
  const sectionGap = 22
  const maxAlignedT = aligned[aligned.length - 1]?.sessionTimeMs ?? 0
  const maxRawPhysicalT = Math.max(
    0,
    ...(rawPhysical ?? []).map((p) => (typeof p.sessionTimeMs === 'number' ? p.sessionTimeMs : 0)),
  )
  const maxT = Math.max(maxAlignedT, maxRawPhysicalT, 1)
  const sx = (t: number) => xPadL + (t / Math.max(1, maxT)) * (totalW - xPadL - xPadR)

  const physicalKeys = selectPhysicalKeys(rawPhysical, aligned)
  const debug = useMemo(() => buildTimelineDebugInfo(rawPhysical, physicalKeys), [rawPhysical, physicalKeys])

  useEffect(() => {
    if (!debug) return
    console.groupCollapsed(`[Timeline Debug] ${sessionKey}`)
    console.log('Raw physical keys:', debug.rawKeys)
    console.table(debug.rawStats)
    console.log('Plotted physical keys:', debug.plottedKeys)
    console.table(debug.plottedStats)
    if (debug.numericLikeNotNumeric.length > 0) {
      console.warn('Numeric-like string values found (coerced for plotting):', debug.numericLikeNotNumeric)
    }
    console.groupEnd()
  }, [debug, sessionKey])
  ...
  const physicalPaths = physicalKeys.map((k, i) => {
    const rawSeries = collectRawSeries(rawPhysical, k)
    const vals = rawSeries.map((p) => p.value).filter((x): x is number => Number.isFinite(x))
    ...
    const points = rawSeries
      .map((p) => {
        const y = yMid + amp - ((p.value - minV) / (maxV - minV || 1)) * (2 * amp)
        return `${sx(p.t).toFixed(1)},${y.toFixed(1)}`
      })
      .join(' ')
    return { key: k, color: physicalSeriesColor(k), points, hasData: vals.length > 0, yMid }
  })
  ...
  return (
    <figure className="mp-correlated-chart mp-correlated-chart--expanded">
      ...
      <details className="mp-subsection-collapsible">
        <summary>Timeline debug (physical channels)</summary>
        <p className="mp-text-muted">
          Session <span className="mp-mono-inline">{sessionKey}</span> · Raw keys: {debug.rawKeys.length} · Plotted keys:{' '}
          {debug.plottedKeys.length}
        </p>
        <ul className="mp-bullet-list">
          {debug.rawStats.map((row) => (
            <li key={`dbg-raw-${row.key}`}>
              <span className="mp-mono-inline">{row.key}</span> raw numeric {row.numericCount}
              {row.numericLikeStringCount > 0 ? `, numeric-like strings ${row.numericLikeStringCount}` : ''}
              {row.otherCount > 0 ? `, other ${row.otherCount}` : ''}
            </li>
          ))}
        </ul>
      </details>
...
function selectPhysicalKeys(
  ...
  for (const point of rawPhysical ?? []) {
    for (const [rawKey, rawValue] of Object.entries(point)) {
      if (rawKey === 'sessionTimeMs' || rawKey === 'caseId') continue
      if (rawKey === 'mentalLoad' || rawKey === 'autonomicState' || rawKey === 'autonomicStability') continue
      if (rawKey === 'cognitiveReadiness' || rawKey === 'activityLevel') continue
      if (coerceNumeric(rawValue) !== null) keySet.add(rawKey)
    }
  }
  for (const row of aligned) {
    for (const [k, v] of Object.entries(row.physical)) {
      if (typeof v === 'number') keySet.add(k)
    }
  }
  const canonical = [
    'hrvRmssd',
  ...
  const canonicalOrdered = canonical
  const extras = Array.from(keySet).filter((k) => !canonical.includes(k)).sort()
  return [...canonicalOrdered, ...extras]
}

function collectRawSeries(rawPhysical: PhysicalTimePoint[] | undefined, key: string): Array<{ t: number; value: number }> {
  const out: Array<{ t: number; value: number }> = []
  for (const point of rawPhysical ?? []) {
    const t = typeof point.sessionTimeMs === 'number' && Number.isFinite(point.sessionTimeMs) ? point.sessionTimeMs : null
    if (t === null) continue
    const value = coerceNumeric((point as Record<string, unknown>)[key])
...
function buildTimelineDebugInfo(
  rawPhysical: PhysicalTimePoint[] | undefined,
  physicalKeys: string[],
) {
  const skip = new Set(['sessionTimeMs', 'caseId', 'mentalLoad', 'autonomicState', 'autonomicStability', 'cognitiveReadiness'])
  const rawMap = new Map<string, { numericCount: number; numericLikeStringCount: number; otherCount: number }>()

  for (const point of rawPhysical ?? []) {
    for (const [key, value] of Object.entries(point)) {
      if (skip.has(key)) continue
      const current = rawMap.get(key) ?? { numericCount: 0, numericLikeStringCount: 0, otherCount: 0 }
      if (typeof value === 'number' && Number.isFinite(value)) current.numericCount += 1
      else if (typeof value === 'string' && value.trim() !== '' && Number.isFinite(Number(value))) current.numericLikeStringCount += 1
      else current.otherCount += 1
      rawMap.set(key, current)
    }
  }

  const plottedMap = new Map<string, number>()
  for (const key of physicalKeys) {
    plottedMap.set(key, collectRawSeries(rawPhysical, key).length)
  }

  const rawStats = Array.from(rawMap.entries())
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([key, counts]) => ({ key, ...counts }))

  const plottedStats = physicalKeys.map((key) => ({
    key,
    plottedNumericCount: plottedMap.get(key) ?? 0,
  }))

  const numericLikeNotNumeric = rawStats
    .filter((x) => x.numericLikeStringCount > 0)
    .map((x) => x.key)

  return {
    rawKeys: rawStats.map((x) => x.key),
    rawStats,
    plottedKeys: physicalKeys,
    plottedStats,
    numericLikeNotNumeric,
  }
}
